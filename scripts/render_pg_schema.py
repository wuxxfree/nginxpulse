...
def main() -> int:
    ...
    template_path = Path(__file__).resolve().parents[1] / "docs" / "pg" / "schema.sql"
    try:
        template = template_path.read_text(encoding="utf-8")
    except Exception as exc:
        print(f"failed to read schema template: {exc}", file=sys.stderr)
        return 1
    ...
    for item in websites:
        ...
        if not name:
            continue
        wid = website_id(name)
        print(f"-- website: {name} ({wid})")
        print(template.replace("{{website_id}}", wid))
        print()
    return 0
